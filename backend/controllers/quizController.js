...
export const submitQuiz = async (req, res) => {
  try {
    ...
    // Add quizId to quizAttempted array (avoid duplicates)
    await Student.findByIdAndUpdate(
      userId,
      { $addToSet: { quizAttempted: quizId } }
    );

    res.status(200).json({ message: "Quiz submission recorded." });
  } catch (error) {
    console.error("Error updating quizAttempted:", error);
    res.status(500).json({ message: "Server error." });
  }
};